...
- feat(whatsapp,sla): aviso automático de requisição vencida pro requisitante — `database/035_sla_breach_notifications.sql` roda via `pg_cron` a cada 4h, detecta tickets vencidos direto em SQL (mesma regra de metas por etapa do Monitor SLA/Analytics: `GESTOR` 24h, `COTAÇÃO` 72h, `APROVAÇÃO` 72h, `COMPRA` 48h, `RECEBIMENTO` 168h) e dispara WhatsApp via `pg_net`/Evolution API direto do Postgres, sem depender do app Node estar de pé. Repete a cada ~20h enquanto o ticket continuar vencido na mesma etapa (nova tabela `sla_notifications` evita duplicar aviso). **Pendente**: `EVOLUTION_APIKEY` precisa ser preenchida na função `private.sla_check_and_notify()` antes de aplicar em produção — migração ainda não aplicada
