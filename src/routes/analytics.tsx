...
const slaTrendConfig: ChartConfig = {
  compliance_rate: { label: "Compliance %", color: "#F5A623" },
};
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="text-center -mt-20">
        ...
        <Badge variant="outline" className="ml-2 text-[10px]" style={{ borderColor: color, color }}>
          {label}
        </Badge>
      </div>
    </div>
...
const FEED_META: Record<
  string,
  { label: string; icon: typeof Package; bg: string; fg: string; badge: string }
> = {
  SUBMITTED: {
  ...
  WINNER_SELECTED: {
    label: "Vencedor Selecionado",
    icon: Trophy,
    bg: "bg-yellow-100",
    fg: "text-yellow-700",
    badge: "bg-yellow-50 text-yellow-700 border-yellow-200",
  },
  M1_ITEMS_QUOTE_COMPLETED: {
    label: "Cotação de Itens",
    icon: FileText,
    bg: "bg-purple-100",
    fg: "text-purple-600",
    badge: "bg-purple-50 text-purple-600 border-purple-200",
...
function AnalyticsPage() {
  ...
  const [compareMode, setCompareMode] = useState<
    "none" | "previous_period" | "same_period_last_year"
  >("none");
  const [activeTab, setActiveTab] = useState<"executive" | "sla" | "financial" | "operational">(
    "executive",
  );
  ...
  const [exportReportType, setExportReportType] = useState<
    "executive" | "sla" | "financial" | "operational"
  >("executive");
  ...
  const buildRows = useCallback((): {
    ...
    if (exportReportType === "executive") {
      return {
        ...
        rows: d.volumeTrend.map(
          (r) => [r.month, r.M1, r.M2, r.M3, r.M4, r.M5, r.M6] as (string | number)[],
        ),
      };
    }
  ...
  const handleExportGenerate = async () => {
    ...
    try {
      ...
      if (exportFormat === "PDF") {
        ...
        doc.setFontSize(9);
        doc.text(
          `Período: ${PERIOD_LABELS[period]} · Módulo: ${moduleFilter} · Gerado em ${now.toLocaleString("pt-BR")}`,
          14,
          25,
        );
        doc.setFontSize(10);
  ...
  const activeModules =
    moduleFilter === "Todos"
      ? (["M1", "M2", "M3", "M4", "M5", "M6"] as const)
      : ([moduleFilter] as const);
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <Link
            ...
            search={{
              ticket: undefined,
              module: moduleFilter !== "Todos" ? moduleFilter : undefined,
            }}
            ...
            <div className="grid lg:grid-cols-3 gap-4">
              <Card className="lg:col-span-2 card-hover-yellow">
                <CardHeader className="pb-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    {moduleFilter !== "Todos" ? ` · ${moduleFilter}` : " por módulo"}
                  </p>
                </CardHeader>
                <CardContent>
              ...
              <Card className="card-hover-yellow">
                <CardHeader className="pb-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Meta: ≥ 95% · etapas concluídas dentro do prazo
                  </p>
                </CardHeader>
                      ...
                      <ChartContainer config={pieChartConfig} className="h-[250px] w-full">
                        <PieChart>
                          ...
                          <Pie
                            ...
                            labelLine={false}
                            onClick={(entry) =>
                              goToModule((entry as unknown as { key: string }).key)
                            }
            ...
            <div className="grid lg:grid-cols-2 gap-4">
              <Card className="card-hover-yellow">
                <CardHeader className="pb-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Compliance % das etapas concluídas no período
                  </p>
                </CardHeader>
                  ...
                  {data.approvalsByLevel.every((l) => l.count === 0) ? (
                    ...
                    <>
                      <ChartContainer config={levelChartConfig} className="h-[190px] w-full">
                        <BarChart
                          data={data.approvalsByLevel.map((l) => ({
                            ...l,
                            label: `${l.level}ª Alçada`,
                          }))}
                          margin={{ top: 5, right: 10, left: -20, bottom: 0 }}
                        >
                          <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                  ...
                  {[
                    {
                      ...
                      value:
                        data.quality.approvalRate != null ? `${data.quality.approvalRate}%` : "—",
                      icon: CheckCircle2,
                    },
                    {
                      label: "Requisições Reprovadas",
                      value: String(data.quality.rejectedCount),
                      icon: AlertTriangle,
                    },
                    {
                      label: "Requisições Editadas",
                      value: String(data.quality.editedCount),
                      icon: RotateCcw,
                    },
                  ].map((m) => (
                ...
                <CardContent className="space-y-4">
                  {[
                    { label: "Compras Concluídas", value: String(data.efficiency.purchasesCount) },
                    {
                      ...
                      value: String(data.efficiency.quotationsCompleted),
                    },
                    {
                      label: "Tempo Médio de Cotação",
                      value:
                        data.efficiency.avgQuotationHours != null
                          ? `${data.efficiency.avgQuotationHours}h`
                          : "—",
                    },
                  ].map((m) => (
                    <div key={m.label} className="flex items-center gap-3">
          ...
          <div className="space-y-4">
            <Card className="card-hover-yellow">
              <CardHeader className="pb-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Compliance % últimos 6 meses (dados reais)
                </p>
              </CardHeader>
              <CardContent>
            ...
            <div className="grid lg:grid-cols-2 gap-4">
              <Card className="card-hover-yellow">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-semibold">
                    Duração por Estágio (detalhada)
                  </CardTitle>
                  <p className="text-xs text-muted-foreground">
                    Média · Mediana · P95 vs Meta — {PERIOD_LABELS[period]}
                  </p>
                </CardHeader>
                <CardContent className="space-y-3">
                    ...
                    .map((s) => (
                      <div key={s.stage} className="space-y-1">
                        ...
                        <div className="flex gap-4 text-[10px] text-muted-foreground">
                          <span>Média: {s.avg}h</span>
                          <span>Mediana: {s.median}h</span>
                          <span>Meta: {s.target}h</span>
                        </div>
                        <Progress
                          value={Math.min((s.avg / s.target) * 100, 100)}
                          className="h-1.5"
                        />
                      </div>
                    ))}
              ...
              <Card className="card-hover-yellow">
                <CardHeader className="pb-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {data.bottlenecks.length} requisição(ões) acima da meta do estágio atual
                  </p>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  {data.bottlenecks.map((b) => (
                    ...
                    >
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        {b.stage} · Solicitante: {b.requester}
                      </p>
                      <Progress
                        value={Math.min((b.hours / b.target) * 100, 100)}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                {
                  label: "% Economia",
                  value: data.financial.savingsPct != null ? `${data.financial.savingsPct}%` : "—",
                  highlight: true,
                },
              ].map((item) => (
                <Card key={item.label} className="card-hover-yellow">
                  <CardContent className="p-4">
                    ...
                    <p
                      className={`text-lg font-bold mt-1 ${item.highlight ? "text-emerald-600" : "text-foreground"}`}
                    >
                      {item.value}
                    </p>
                  </CardContent>
            ...
            <div className="grid lg:grid-cols-2 gap-4">
              <Card className="card-hover-yellow">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-semibold">
                    Economia Mensal em Cotações
                  </CardTitle>
                  <p className="text-xs text-muted-foreground">
                    Proposta mais cara vs valor contratado
                  </p>
                </CardHeader>
                <CardContent>
                  {data.financial.monthlySavings.every((m) => m.original === 0) ? (
                    ...
                    <ChartContainer config={savingsChartConfig} className="h-[220px] w-full">
                      <BarChart
                        data={data.financial.monthlySavings}
                        margin={{ top: 5, right: 10, left: -10, bottom: 0 }}
                      >
                        <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                        <XAxis
                ...
                <CardContent className="space-y-3">
                  ...
                  {data.financial.spendByModule.map((c) => (
                    <div key={c.module} className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-muted-foreground">
                          {c.module} — {c.label} · {c.count} compra(s)
                        </span>
                        <span className="font-medium text-foreground">
                          {fmtBRL(c.value)} ({c.pct}%)
                        </span>
                      </div>
                ...
                {data.financial.topSuppliers.map((s, i) => (
                  <div key={s.name} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        {s.count} pedido(s) · {fmtBRL(s.value)}
                      </p>
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-4">
            <Card className="card-hover-yellow">
              <CardHeader className="pb-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Compras confirmadas — {PERIOD_LABELS[period]}
                </p>
              </CardHeader>
              <CardContent>
                {data.topBuyers.length === 0 ? (
                ...
                <CardContent className="space-y-3">
                  ...
                  {data.bottlenecks.map((b) => (
                    ...
                    >
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        {b.stage} · {b.requester} · {b.hours}h no estágio
                      </p>
                    </button>
                  ))}
                </CardContent>
      ...
      <div className="max-w-6xl mx-auto mt-6 space-y-4">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-border">
              ...
              <div className="p-3 text-center">
                ...
                <p className="text-lg font-bold text-foreground">
                  {fmtBRL(data.live.valueApprovedToday)}
                </p>
              </div>
              <div className="p-3 text-center">
                <p className="text-[10px] text-muted-foreground font-medium">Gargalos Ativos</p>
                <p
                  className={`text-lg font-bold ${data.live.activeBottlenecks > 3 ? "text-red-500" : "text-foreground"}`}
                >
                  {data.live.activeBottlenecks}
                </p>
              </div>
              <div className="p-3 text-center">
                <p className="text-[10px] text-muted-foreground font-medium">SLA Compliance</p>
                <p
                  className={`text-lg font-bold ${data.live.slaCompliance == null || data.live.slaCompliance >= 85 ? "text-foreground" : "text-red-500"}`}
                >
        ...
        <Card className="card-hover-yellow">
          <CardHeader className="pb-2">
            ...
            <p className="text-xs text-muted-foreground">
              Últimos eventos reais registrados no sistema
            </p>
          </CardHeader>
          <CardContent>
            {data.feed.length === 0 ? (
                ...
                {data.feed.map((evt) => {
                  ...
                  return (
                    <div key={evt.id} className="flex items-start gap-3 rounded-lg border p-3">
                      <div
                        className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-md ${meta.bg}`}
                      >
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {!exportDone ? (
            <div className="space-y-5 mt-2">
              <div className="space-y-2">
                ...
                <RadioGroup
                  ...
                  onValueChange={(v) => setExportReportType(v as typeof exportReportType)}
                ...
                >
                  {[
                    ...
                    <Label
                      ...
                      className={`flex items-center gap-2 rounded-lg border-2 p-3 cursor-pointer transition-all hover:border-[var(--vp-yellow)] ${exportReportType === opt.value ? "border-[var(--vp-yellow)] bg-accent" : "border-border"}`}
                    >
                      <RadioGroupItem
                        value={opt.value}
                        id={`rt-${opt.value}`}
              ...
              <div className="space-y-2">
                ...
                >
                  {[
                    ...
                    <Label
                      key={opt.value}
                      htmlFor={`ef-${opt.value}`}
                      className={`flex flex-col items-center gap-1 rounded-lg border-2 p-3 cursor-pointer transition-all hover:border-[var(--vp-yellow)] ${exportFormat === opt.value ? "border-[var(--vp-yellow)] bg-accent" : "border-border"}`}
                    >
                      <RadioGroupItem
                        value={opt.value}
                        id={`ef-${opt.value}`}
              ...
              <div className="rounded-lg bg-muted/50 p-3 text-xs text-muted-foreground">
                📋 Período: {PERIOD_LABELS[period]} · Módulo:{" "}
                {moduleFilter === "Todos" ? "Todos" : moduleFilter}
              </div>
              <Button
                className="w-full gap-2"
                onClick={() => void handleExportGenerate()}
            ...
            <div className="space-y-4 mt-2">
              ...
              <Button variant="outline" className="w-full" onClick={() => setExportDone(null)}>
                Novo Relatório
              </Button>
            </div>
          )}
