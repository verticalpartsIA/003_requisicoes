...
function RentalPage() {
  ...
  const categoryLabel = useMemo(
    () =>
      categories
        .map((v) => EQUIPMENT_CATEGORIES.find((c) => c.value === v)?.label ?? v)
        .join(" + ") || "Locação",
    [categories],
  );
  ...
  useEffect(() => {
    try {
      ...
      if (typeof s.needsSecurityInduction === "boolean")
        setNeedsSecurityInduction(s.needsSecurityInduction);
  ...
  const resetForm = () => {
    ...
    setProjectNumber("");
    setArtStatus("EMITIR");
    setNeedsSecurityInduction(false);
    setClientNormFile(null);
    setEditClientNormPath(null);
  ...
  const validateStep = (): boolean => {
    ...
    if (step === 2) {
      ...
      if (isLongRental && justification.length < 50) {
        toast.error(
          "Locação acima de 30 dias requer justificativa detalhada (mín. 50 caracteres).",
        );
        return false;
      }
    }
    ...
    try {
      ...
      if (created?.id) {
        const { error: auditError } = await supabaseBrowser.from("audit_logs").insert({
          ...
          details: { module: "M6", urgency: urgencyLevel },
        });
          ...
          {step === 0 && (
            <div className="space-y-4">
                ...
                <div className="space-y-3 rounded-lg border p-3 bg-muted/20">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium">
                      A ART (Anotação de Responsabilidade Técnica) já existe?
                    </label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium">
                      Tem alguma norma específica do cliente?
                    </label>
                    <input
                      type="file"
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  placeholder="Capacidade, potência, dimensões, requisitos especiais..."
                  value={specs}
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  className={cn(
                    stepAttempted && (!quantity || parseInt(quantity) <= 0) && FIELD_ERROR_CLASS,
                  )}
                />
              </div>
              <div className="space-y-1.5">
                  ...
                  <Popover open={endDateOpen} onOpenChange={setEndDateOpen}>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        ...
                        disabled={(d) => d < startOfDay(startDate || new Date())}
                        initialFocus
                        className="p-3 pointer-events-auto"
                        locale={ptBR}
                      />
                ...
                >
                  {isLongRental ? (
                    <p className="flex items-center gap-1.5 font-medium">
                      ...
                      Locação longa: <strong>{rentalDays} dias</strong> — requer contrato especial e
                      justificativa detalhada
                    </p>
                  ) : (
                    <p>
                      Período:{" "}
                      <span className="font-semibold text-foreground">{rentalDays} dia(s)</span>
                    </p>
                  )}
                </div>
              )}
                ...
                <label className="text-sm font-medium">
                  ...
                  {isLongRental && (
                    <span className="text-orange-600 ml-1">
                      (mín. 50 caracteres — locação longa)
                    </span>
                  )}
                </label>
                <Textarea
                  ...
                  className={cn(
                    stepAttempted &&
                      (justification.length < 10 || (isLongRental && justification.length < 50)) &&
                      FIELD_ERROR_CLASS,
                  )}
                />
                <p className="text-[11px] text-muted-foreground">
                  {justification.length}/500{isLongRental && " (mín. 50)"}
                </p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="rounded-lg border border-vp-yellow/40 bg-amber-50/30 p-3">
                <p className="text-xs text-vp-yellow-dark font-medium">
                  Confira os dados abaixo antes de {editMode ? "salvar" : "enviar"}. Use "Voltar"
                  para corrigir qualquer campo.
                </p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm font-medium">
                  {categories.length > 0
                    ? categories
                        .map((c) => EQUIPMENT_CATEGORIES.find((e) => e.value === c)?.label ?? c)
                        .join(", ")
                    : "—"}
                </p>
              </div>
